...
class TagsSerializer(serializers.ModelSerializer):        
    url = serializers.HyperlinkedRelatedField(view_name="menu_plan:tags-detail", read_only=True, lookup_field="tags")
    class Meta:
        model = Tags
        fields = [ 'url','name', 'id']

class AllergensSerializer(serializers.ModelSerializer):        
    url = serializers.HyperlinkedRelatedField(view_name="menu_plan:allergen-detail", read_only=True, lookup_field="allergen")    
    class Meta:
        model = Allergen
        fields = [ 'url','name', 'id']
...
class UtensilsSerializer(serializers.ModelSerializer):        
    url = serializers.HyperlinkedRelatedField(view_name="menu_plan:utensil-detail", read_only=True, lookup_field="utensil")   
    class Meta:
        model = Utensil
        fields = [ 'url','name', 'id']

class IngredientsSerializer(serializers.ModelSerializer):
    url = serializers.HyperlinkedRelatedField(view_name="menu_plan:ingredients-detail", read_only=True, lookup_field="ingredients")
    class Meta:
        model = Ingredients
...
class NutritionalvaluesSerializer(serializers.ModelSerializer):
    url = serializers.HyperlinkedRelatedField(view_name="menu_plan:nutritionalvalue-detail", read_only=True, lookup_field="nutritionalvalue")
    recipe = serializers.HyperlinkedRelatedField(view_name="menu_plan:recipe-detail", read_only=True, source="recipe_name" )
...
class InstructionsSerializer(serializers.ModelSerializer):
    url = serializers.HyperlinkedRelatedField(view_name="menu_plan:instruction-detail", read_only=True, lookup_field="instruction")
    recipe = serializers.HyperlinkedRelatedField(view_name="menu_plan:recipe-detail", read_only=True, source="recipe_name" )
    class Meta:
        model = Instruction
...
class  RecipeSerializer(serializers.HyperlinkedModelSerializer):
    ...
    ingredients = IngredientsSerializer(allow_null=True, read_only=True, many=True, )
    nutritionalvalue = NutritionalvaluesSerializer(allow_null=True, read_only=True,  )
    instruction_set = InstructionsSerializer(allow_null=True, read_only=True, many=True )
    url = serializers.HyperlinkedRelatedField(view_name="menu_plan:recipe-detail", read_only=True, lookup_field="pk")
    ...
    def create(self, validated_data):
        ...
        if 'allergen' in Data:
            ...
            for allergen_data in AllergensData:
                Allergen_Obj, created = Allergen.objects.get_or_create(name=allergen_data)
        ...
        if 'utensil' in Data:
            ...
            for Utensil_data in UtensilsData:
                Utensils_Obj, created = Utensil.objects.get_or_create(name=Utensil_data)
        ...
        if 'nutritionalvalues' in Data:
            NutritionalValuesData = ast.literal_eval(str(Data['nutritionalvalues']))  
        ...
        if 'ingredients' in Data:
            IngredientsData = ast.literal_eval(inspect.cleandoc(Data['ingredients']))
        ...
        if 'instructions' in Data:
            InstructionsData = ast.literal_eval(inspect.cleandoc(Data['instructions']))
